...
class Model {
  ...
  static refreshAttributes() {
    ...
    const customGetters = new Map();
    const customSetters = new Map();

    _.forEach(Object.getOwnPropertyDescriptors(this.prototype), ({ get, set }, key) => {
      // skip attribute accessors
      if (get && get.length === 0 && get.toString().startsWith(`get ${key}`)) {
        customGetters.set(key, get);
      }
      if (set && set.length === 1 && set.toString().startsWith(`set ${key}`)) {
        customSetters.set(key, set);
      }
    });

    ['get', 'set'].forEach(type => {
      const funcs = {};
      const custom = type === 'get' ? customGetters : customSetters;

      _.each(this.rawAttributes, (options, key) => {

        if (options[type] && custom.has(key)) {
          throw new Error(`Cannot declare attribute '${key}', conflicting ${type}ter defined on model`);
        }

        if (options.hasOwnProperty(type)) {
          custom.set(key, options[type]);
        }
    ...
    this.prototype._customGetters = customGetters;
    this.prototype._customSetters = customSetters;
  ...
  get(key, options) {
    ...
    if (key) {
      const getter = this._customGetters.get(key);
      if (getter && !options.raw) {
    ...
    if (
      this._customGetters.size > 0 ||
      options.plain && this._options.include
      || options.clone
    ) {
      const values = {};

      for (const getterKey of this._customGetters.keys()) {
        if (
          this._options.attributes
          && !this._options.attributes.includes(getterKey)
        ) {
          continue;
        }
        values[getterKey] = this.get(getterKey, options);
      }

      for (const dataKey in this.dataValues) {
        if (!values.hasOwnProperty(dataKey) && this.dataValues.hasOwnProperty(dataKey)) {
          values[dataKey] = this.get(dataKey, options);
        }
      }
  ...
  set(key, value, options) {
    ...
    if (!options.raw && this._customSetters.has(key)) {
      this._customSetters.get(key).call(this, value, key);
